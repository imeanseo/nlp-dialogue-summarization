...
def compute_metrics(eval_pred):
    ...
    for token in remove_tokens:
        replaced_preds = [s.replace(token, '') for s in replaced_preds]
        replaced_labels = [s.replace(token, '') for s in replaced_labels]
    
    # 빈 문자열 처리
    replaced_preds = [p if p.strip() else "empty" for p in replaced_preds]
    replaced_labels = [l if l.strip() else "empty" for l in replaced_labels]
