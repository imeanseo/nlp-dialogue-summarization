...
if len(sys.argv) > 1:
    ...
    checkpoint_name = 'best_model'  # 기본

model_path = os.path.join(config['general']['output_dir'], checkpoint_name)

if not os.path.exists(model_path):
    print(f"⚠️ {checkpoint_name} 없음")
    all_checkpoints = glob.glob(os.path.join(config['general']['output_dir'], "*"))
    print("\n사용 가능한 체크포인트:")
    for cp in all_checkpoints:
        print(f"  - {os.path.basename(cp)}")
    exit(1)

print(f"✅ 로드: {os.path.basename(model_path)}")

tokenizer = AutoTokenizer.from_pretrained(model_path, src_lang='ko_KR', tgt_lang='ko_KR')
model = AutoModelForSeq2SeqLM.from_pretrained(model_path)
model.to(device)
model.eval()
print("✅ 모델 로드 완료")
...
# test_466 누락 확인 및 추가
if 'test_466' not in test_df['fname'].values:
    print("⚠️ test_466 누락 감지 → 더미 행 추가")
    dummy_row = pd.DataFrame({
        'fname': ['test_466'],
        'dialogue': ['#Person1#: 안녕하세요. #Person2#: 반갑습니다. #Person1#: 오늘 날씨가 좋네요. #Person2#: 네 정말 그렇습니다.']
    })
    test_df = pd.concat([test_df, dummy_row], ignore_index=True)
...
def generate_summary(dialogue, model, tokenizer, device, config):
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs,
            max_length=config['inference']['generate_max_length'],      # ← 그냥 읽기
            min_length=config['inference']['min_length'],               # ← 그냥 읽기
            num_beams=config['inference']['num_beams'],
            length_penalty=config['inference']['length_penalty'],       # ← 그냥 읽기
...
version = 'v2.5'
model_name = 'mbart'
date_str = datetime.now().strftime('%m%d_%H%M')  # ← MMDD_HHMM 형식
...
submission.to_csv(
    submission_path, 
    index=False,
    quoting=csv.QUOTE_NONE,          # ← 따옴표 없앰               # ← 특수문자 escape용
    encoding='utf-8-sig'
)
